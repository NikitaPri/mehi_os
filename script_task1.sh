...
main () {
	if [ "$1" == "--help" ]
	then
        	echo "При помощи данного сценария производится управление пользователями и группами, а так же их поиск."
	else
	...
	do
		case $opt in
			...
			"Справка")
				...
				echo "управление пользователями включает: добавление, удаление, блокировку, смену пароля и добавление в группу пользователя."
				echo "управление группами включает: добавление, удаление группы, добавление пользователя в группу и удаление пользователя из группы."
				echo "поиск пользователей или групп включает: поиск пользователя и поиск группы."
				menushow "${options[@]}"
				;;
			"Выход из программы")
				return 0
